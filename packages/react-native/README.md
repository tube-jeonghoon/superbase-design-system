# @thesuperbase/react-native

React Native용 컴포넌트. `@thesuperbase/react`(웹)와 거의 동일한 컴포넌트 API(Text/Button/TextField/Stack/Switch)를 제공하며 `@thesuperbase/tokens`의 native 토큰을 소비한다.

RN 관용에 따른 일부 차이: 콜백은 RN 방식(Button `onPress`, TextField `onChangeText`)을 따르고, `Button`의 `children`은 단일 텍스트(`string`)다(웹은 `ReactNode`).

## 사용

    import { Button, Text } from "@thesuperbase/react-native";
...
시뮬레이터 없이 `react-native-web` + jsdom으로 헤드리스 단위 테스트한다:
    ...
    pnpm --filter @thesuperbase/react-native test
